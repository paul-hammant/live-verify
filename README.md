...
available for web/JavasScript via WASM) that is not in the most recent C++ version. [See proof](https://github.com/paul-hammant/live-verify/issues/1). 
...
**Example `.verification-meta.json` for tradesperson accountability:**

The verification authority (e.g., competent-person-schemes.gov.uk, checkatrade.com) can include programmatic action suggestions in their `.verification-meta.json` response to encourage work registration and create paper trails:
...
**Privacy note:** The `redirectParams` can be optional - homeowner chooses whether to actually register the visit. The verification itself doesn't send data to tax authorities; the `.verification-meta.json` response merely *suggests* registration as an option via programmatic redirect.
